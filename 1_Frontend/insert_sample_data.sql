...
-- 관리자 계정 ID를 가져와서 사용 (admin 권한 있는 첫 번째 사용자)
DO $$
DECLARE
    admin_user_id UUID;
    sample_user_id UUID;
BEGIN
    ...
    IF sample_user_id IS NOT NULL THEN
        ...
        (sample_user_id, 'system', '🎉 PoliticianFinder에 오신 것을 환영합니다', '회원가입을 축하드립니다! PoliticianFinder에서 우리 지역 정치인을 만나보세요.', '/politicians', false, NOW() - INTERVAL '1 hour'),
        (sample_user_id, 'system', '📢 새로운 공지사항이 등록되었습니다', '서비스 이용약관 변경 안내 - 2025년 2월 1일부터 변경된 약관이 적용됩니다.', '/notices', false, NOW() - INTERVAL '3 hours'),
        (sample_user_id, 'comment', '💬 새 댓글이 달렸습니다', '회원님이 작성한 게시글에 새로운 댓글이 달렸습니다.', '/community/posts', false, NOW() - INTERVAL '5 hours'),
        (sample_user_id, 'post_like', '👍 게시글에 공감을 받았습니다', '회원님의 게시글에 3명이 공감을 표시했습니다.', '/community/posts', false, NOW() - INTERVAL '8 hours'),
        (sample_user_id, 'system', '🔔 알림 설정을 확인해주세요', '알림 수신 설정을 통해 원하는 알림만 받아보실 수 있습니다.', '/settings', true, NOW() - INTERVAL '1 day'),
        (sample_user_id, 'follow', '✨ 새로운 정치인 활동', '관심 정치인이 새로운 게시글을 작성했습니다.', '/politicians', true, NOW() - INTERVAL '2 days'),
        (sample_user_id, 'system', '📱 모바일 앱 출시 예정 안내', 'PoliticianFinder 모바일 앱이 3월에 출시됩니다!', '/notices', true, NOW() - INTERVAL '3 days');
    END IF;
END $$;
