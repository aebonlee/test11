...
-- user_id는 auth.users 테이블의 실제 ID를 사용해야 함
DO $$
DECLARE
    target_auth_id UUID;
BEGIN
    ...
    IF target_auth_id IS NOT NULL THEN
        ...
        (target_auth_id, 'system', '🎉 PoliticianFinder에 오신 것을 환영합니다', '회원가입을 축하드립니다! PoliticianFinder에서 우리 지역 정치인을 만나보세요.', '/politicians', false, NOW() - INTERVAL '1 hour'),
        (target_auth_id, 'system', '📢 새로운 공지사항이 등록되었습니다', '서비스 이용약관 변경 안내 - 2025년 2월 1일부터 변경된 약관이 적용됩니다.', '/notices', false, NOW() - INTERVAL '3 hours'),
        ...
        (target_auth_id, 'system', '🔔 알림 설정을 확인해주세요', '알림 수신 설정을 통해 원하는 알림만 받아보실 수 있습니다.', '/settings', true, NOW() - INTERVAL '1 day'),
        (target_auth_id, 'follow', '✨ 새로운 정치인 활동', '관심 정치인이 새로운 게시글을 작성했습니다.', '/politicians', true, NOW() - INTERVAL '2 days'),
        (target_auth_id, 'system', '📱 모바일 앱 출시 예정 안내', 'PoliticianFinder 모바일 앱이 3월에 출시됩니다!', '/notices', true, NOW() - INTERVAL '3 days');
        
        RAISE NOTICE 'Inserted notifications for auth user: %', target_auth_id;
    ELSE
        RAISE NOTICE 'No auth.users found. Please create a user account first.';
    END IF;
END $$;
