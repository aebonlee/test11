 ...
 *
 * 실행: npx ts-node scripts/mobile-screenshot.ts
 */
...
const MOBILE_DEVICES = [
  ...
  { name: 'Galaxy-S23', device: devices['Galaxy S9+'] }, // S23 대신 S9+ 사용 (유사 크기)
];
...
const issues: { page: string; description: string; severity: 'high' | 'medium' | 'low' }[] = [];
...
async function setDarkMode(page: Page, enabled: boolean) {
  ...
  await delay(500); // 테마 변경 적용 대기
}

async function checkPageIssues(page: Page, pageName: string): Promise<string[]> {
  const pageIssues: string[] = [];

  // 1. 가로 스크롤 체크 (모바일에서 가로 스크롤은 UX 문제)
  ...
  // 3. 텍스트 크기 체크 (12px 미만은 읽기 어려움)
  const smallText = await page.evaluate(() => {
    const allText = document.querySelectorAll('p, span, div, a, button, label, h1, h2, h3, h4, h5, h6');
    let smallCount = 0;
    allText.forEach((el) => {
    ...
    for (const pageConfig of PAGES_TO_CAPTURE) {
      ...
      for (const theme of ['light', 'dark'] as const) {
        try {
          ...
          console.log(`    ❌ ${theme} 모드 실패: ${error}`);
          issues.push({
            page: pageConfig.name,
            description: `페이지 로드 실패: ${error}`,
            severity: 'high'
          });
        }
      }
...
function generateReport(filePath: string, timestamp: string) {
...
`;
  ...
  // 페이지별로 그룹화
  const pageGroups = new Map<string, ScreenshotResult[]>();
  results.forEach(r => {
    const key = r.page;
