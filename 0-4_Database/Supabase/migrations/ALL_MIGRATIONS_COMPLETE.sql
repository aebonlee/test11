-- ============================================================
-- 전체 마이그레이션 통합 스크립트 (완전 버전)
-- Migration 053 + Migration 054 (COMPLETE - 모든 케이스 처리)
...
-- ============================================================
-- Migration 054: posts/comments 스키마 수정 (완전 버전)
-- ============================================================

-- STEP 1: politician_id 컬럼 추가 (없을 경우 대비)
ALTER TABLE posts
  ADD COLUMN IF NOT EXISTS politician_id TEXT REFERENCES politicians(id) ON DELETE SET NULL;

ALTER TABLE comments
  ADD COLUMN IF NOT EXISTS politician_id TEXT REFERENCES politicians(id) ON DELETE SET NULL;

-- STEP 2: NOT NULL 제약조건 제거
ALTER TABLE posts
...
DO $$
BEGIN
  -- posts 테이블: politician_id가 있는 행의 user_id를 NULL로 설정
  UPDATE posts
  SET user_id = NULL
  WHERE politician_id IS NOT NULL;

  -- comments 테이블: politician_id가 있는 행의 user_id를 NULL로 설정
  UPDATE comments
  SET user_id = NULL
  WHERE politician_id IS NOT NULL;
END $$;

-- STEP 4: author_type 컬럼 추가 및 설정
ALTER TABLE posts
