...
V13.0은 Prior 6.5, Coefficient 0.5를 사용하는 점수 계산 알고리즘입니다.
...
- **최저**: 300점 (모든 카테고리 -7: 30×10)
- **중립**: 675점 (모든 카테고리 0: 67.5×10)
- **최고**: 1,000점 (모든 카테고리 +7: 100×10)

---

## 5. 등급 매핑

### 8단계 금속 등급 (300~1,000점 범위)
...
# 최종 점수
final_score = sum(category_scores)  # 약 650점
grade = 'G'  # Gold 등급
```
...
```python
# 모든 카테고리 평균 +3
category_score = (6.5 + 3 * 0.5) * 10  # 80점
...
1. **등급 범위 검증**: -7 ≤ rating ≤ +7
2. **데이터 개수 검증**: 각 카테고리 최소 50개
3. **점수 범위 검증**: 400 ≤ final_score ≤ 1,000

### 이상치 처리

- 등급이 -7~+7 범위를 벗어나는 데이터는 제외
