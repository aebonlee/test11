...
def main():
    print("="*80)
    print("데이터 풀링 시스템 - 전체 카테고리 평가")
    print("="*80)
    print(f"정치인: {POLITICIAN_NAME}")
    print(f"카테고리: 9개 (Leadership ~ PublicInterest)")
    print(f"예상 소요 시간: 약 {9 * 22}분 (약 {9 * 22 / 60:.1f}시간)")
    print("="*80)
    ...
    for idx, (category_eng, category_kor) in enumerate(CATEGORIES, 1):
        print(f"\n[{idx}/9] {category_eng} ({category_kor}) 시작...")
    ...
    for result in all_results:
        ...
        print(f"  ChatGPT: {result['scores']['ChatGPT']['category_score']:.1f}점")
        print(f"  Grok:    {result['scores']['Grok']['category_score']:.1f}점")
        print(f"  Claude:  {result['scores']['Claude']['category_score']:.1f}점")
        print(f"  최종:    {result['final_score']:.1f}점")
        print(f"  소요시간: {result['duration_minutes']:.1f}분")
    ...
    # 전체 결과 저장
    summary_file = f"pooling_all_categories_summary_{POLITICIAN_NAME}.json"
