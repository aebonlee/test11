...
**생성할 파일:**
1. `scripts/seed/seed-politicians.ts` - 정치인 데이터 시딩 스크립트
2. `package.json` - seed 스크립트 추가

**데이터 소스:**
- P4BA1에서 크롤링한 JSON 파일
- 또는 수동으로 준비한 초기 데이터
...
{
  "scripts": {
    "seed:politicians": "tsx scripts/seed/seed-politicians.ts"
  }
}
```
...
**절대 경로 별칭**: `@/` (예: `import ... from '@/scripts/seed/seed-politicians'`)
...
1. 크롤링 데이터 로드
2. politicians 테이블 INSERT
3. careers, pledges 테이블 INSERT
4. 중복 체크 로직
5. 시딩 스크립트 실행 명령
