# 작업지시서: P3BA11

## 📋 기본 정보

- **작업 ID**: P3BA11
- **업무명**: AI 평가 조회 API (Mock → Real)
- **Phase**: Phase 3
- **Area**: Backend APIs (BA)
- **서브 에이전트**: backend-developer
- **작업 방식**: AI-Only

---

## 🎯 작업 목표

Mock API를 Supabase 연동 Real API로 전환하여 실제 AI 평가 데이터 조회 기능 구현

---

## 🔧 사용 도구

```
[Claude 도구]
Read, Edit, Write, Grep, Glob, Bash

[기술 스택]
TypeScript, Next.js API Routes, Supabase, Zod

[전문 스킬]
api-builder, api-test
```

**도구 설명**:
- **Claude 도구**: Claude Code의 기본 기능 (Read, Write, Edit, Bash, Glob, Grep 등)
- **기술 스택**: 프로젝트에 사용되는 프레임워크 및 라이브러리
- **전문 스킬**: Anthropic 빌트인 스킬 (.claude/skills/*.md 참조)

## 🔗 의존성 정보

**의존성 체인**: P2D1, P3BA1, P3BA2

이 작업을 시작하기 전에 다음 작업이 완료되어야 합니다:
- P2D1: Supabase 스키마 구축 (ai_evaluations 테이블)
- P3BA1: 인증 API (사용자 인증)
- P3BA2: 정치인 API (정치인 데이터)

---

## 📦 기대 결과물

- `src/app/api/politicians/[id]/evaluation/route.ts` (Mock → Real 교체)
- `src/app/api/evaluations/[evaluationId]/route.ts` (개별 평가 조회)
- `src/app/api/evaluations/history/route.ts` (평가 이력 조회)

**구현해야 할 세부 항목**:

1. **Mock 코드 제거 및 Real API 구현**
   - 기존 Mock 응답 제거
   - Supabase 클라이언트 연결
   - `ai_evaluations` 테이블 조회

2. **정치인별 AI 평가 조회**
   - 5개 AI 모델 평가 데이터 (Claude, ChatGPT, Gemini, Grok, Perplexity)
   - 10개 평가 기준별 점수 및 근거
   - 종합 점수 및 등급 계산

3. **평가 이력 조회**
   - 시계열 데이터 조회
   - 점수 변화 추이
   - 캐싱 전략 (5분)

4. **에러 처리 및 검증**
   - 존재하지 않는 정치인 처리
   - 평가 데이터 없음 처리
   - RLS 정책 적용 확인

5. **성능 최적화**
   - 인덱스 활용 (politician_id, created_at)
   - 캐싱 전략
   - 병렬 쿼리

---

## 📝 작업 지시사항

### 1. 준비 단계

- 프로젝트 루트: `Developement_Real_PoliticianFinder/1_Frontend`
- 현재 Mock API 파일 확인: `src/app/api/politicians/[id]/evaluation/route.ts`
- DB 스키마 확인: `0-4_Database/Supabase/migrations/011_create_ai_evaluations_table.sql`
- 의존성 작업 완료 확인: P2D1, P3BA1, P3BA2

### 2. 구현 단계

**2.1 정치인별 AI 평가 조회 API**

```typescript
// src/app/api/politicians/[id]/evaluation/route.ts
import { createClient } from '@/lib/supabase/server'
import { NextRequest, NextResponse } from 'next/server'

export async function GET(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  const supabase = createClient()
  const politicianId = params.id

  // 1. 최신 평가 데이터 조회
  const { data: evaluations, error } = await supabase
    .from('ai_evaluations')
    .select('*')
    .eq('politician_id', politicianId)
    .order('created_at', { ascending: false })
    .limit(1)

  // 2. 응답 포맷 변환
  // 3. 에러 처리
}
```

**2.2 개별 평가 조회 API**

```typescript
// src/app/api/evaluations/[evaluationId]/route.ts
// 특정 평가 ID로 상세 조회
```

**2.3 평가 이력 조회 API**

```typescript
// src/app/api/evaluations/history/route.ts
// 시계열 데이터 조회 (최근 30일)
```

### 3. 검증 단계

- 타입 체크: `npm run type-check`
- 빌드: `npm run build`
- API 테스트: Thunder Client 또는 Postman
- RLS 정책 확인

### 4. 완료 단계

- 생성/수정된 파일 목록 확인
- PROJECT GRID 업데이트 (생성된 파일, 작업 시간, 상태)
- 다음 작업 (P3BA12) 준비

---

## ✅ 완료 기준

- [ ] AI 평가 조회 API가 실제 Supabase 데이터를 반환함
- [ ] Mock 코드가 완전히 제거됨
- [ ] 5개 AI 모델 평가 데이터 정상 조회
- [ ] 10개 평가 기준별 점수 및 근거 정상 조회
- [ ] 평가 이력 시계열 데이터 정상 조회
- [ ] TypeScript 타입 체크 통과
- [ ] Next.js 빌드 성공
- [ ] RLS 정책 적용 확인
- [ ] 에러 케이스 처리 완료
- [ ] PROJECT GRID 상태 업데이트 완료

---

## 📊 데이터베이스 스키마 참고

```sql
-- ai_evaluations 테이블
CREATE TABLE ai_evaluations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  politician_id UUID NOT NULL REFERENCES politicians(id),
  evaluator TEXT NOT NULL, -- 'claude', 'chatgpt', 'gemini', 'grok', 'perplexity'
  overall_score INTEGER NOT NULL,
  grade TEXT NOT NULL, -- 'Mugunghwa', 'Rose', 'Maple', 'Orchid', 'Chrysanthemum'

  -- 10개 평가 기준 (각 0-100점)
  integrity_score INTEGER,
  expertise_score INTEGER,
  communication_score INTEGER,
  leadership_score INTEGER,
  transparency_score INTEGER,
  responsiveness_score INTEGER,
  innovation_score INTEGER,
  collaboration_score INTEGER,
  constituency_service_score INTEGER,
  policy_impact_score INTEGER,

  -- 근거 (각 기준별)
  integrity_evidence TEXT,
  expertise_evidence TEXT,
  communication_evidence TEXT,
  leadership_evidence TEXT,
  transparency_evidence TEXT,
  responsiveness_evidence TEXT,
  innovation_evidence TEXT,
  collaboration_evidence TEXT,
  constituency_service_evidence TEXT,
  policy_impact_evidence TEXT,

  report_url TEXT, -- PDF 리포트 URL (추후 구현)
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 인덱스
CREATE INDEX idx_ai_evaluations_politician_id ON ai_evaluations(politician_id);
CREATE INDEX idx_ai_evaluations_created_at ON ai_evaluations(created_at);
```

---

**작업지시서 생성일**: 2025-11-09
**PROJECT GRID Version**: v4.2
