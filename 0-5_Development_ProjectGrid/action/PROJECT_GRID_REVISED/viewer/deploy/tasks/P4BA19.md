...
시계열 AI 평가 데이터를 관리하고, 점수 변화 추이를 조회/분석하는 API 구현
...
**의존성 체인**: P3BA11, P4BA14

이 작업을 시작하기 전에 다음 작업이 완료되어야 합니다:
- P3BA11: AI 평가 조회 API (평가 데이터)
- P4BA14: AI 평가 생성 엔진 (평가 생성)

---

## 📦 기대 결과물

- `src/app/api/evaluations/history/[politicianId]/route.ts` (평가 이력 조회)
- `src/app/api/evaluations/trends/route.ts` (점수 변화 추이)
- `src/app/api/evaluations/compare/route.ts` (정치인 간 비교)
- `src/app/api/evaluations/archive/route.ts` (평가 아카이브)
- `0-4_Database/Supabase/migrations/017_create_evaluation_snapshots.sql` (스냅샷 테이블)

**구현해야 할 세부 항목**:

1. **평가 이력 조회 API**
   - 정치인별 전체 평가 이력
   - 날짜 범위 필터 (최근 30일, 90일, 1년)
   - AI 모델별 필터
...
- DB 스키마 확인: `ai_evaluations` 테이블
- 의존성 작업 완료 확인: P3BA11, P4BA14

### 2. 구현 단계

**2.1 평가 스냅샷 테이블 마이그레이션**
...
export async function POST(request: NextRequest) {
  ...
  if (!politician_ids || politician_ids.length < 2 || politician_ids.length > 5) {
    return NextResponse.json(
      { error: 'politician_ids must contain 2-5 UUIDs' },
      { status: 400 }
    )
  }
