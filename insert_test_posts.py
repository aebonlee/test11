...
# -*- coding: utf-8 -*-
"""
등록된 정치인 ID를 사용하여 테스트 게시글 10개 작성
정치인 게시글 5개 + 일반 회원 게시글 5개
"""
...
# Get actual politician IDs
print("Step 1: Fetching actual politicians...")
politicians_result = supabase.table('politicians').select('id, name, party').limit(10).execute()
politicians = politicians_result.data[:5]  # Use first 5
print(f"Found {len(politicians)} politicians to use\n")

# Create 5 politician posts (using actual politician data)
...
for i, post in enumerate(politician_posts, 1):
    try:
        ...
        print(f"     Author: {post['author']} (ID: {post['politician_id']})")
        print()
    except Exception as e:
        print(f"[{i}/5] Failed: {str(e)}\n")

# Create 5 general member posts (no politician_id)
